...
function readKeyFromFile(fileName: string) {
  try {
    const binary: xChaChaKey | aesKey = JSON.parse(
      readFileSync(fileName, "utf8")
    );
    if (binary.type === "XChaCha20") {
      return {
        inputNonce: Buffer.from(binary.nonce, "base64"),
        inputKey: Uint8Array.from(binary.key.split(","), (byte: string) =>
          parseInt(byte)
        ),
        inputPublicNonce: Buffer.from(binary.publicNonce, "base64"),
      };
    } else {
      return {
        inputKey: Uint8Array.from(binary.key.split(","), (byte: string) =>
...
export {
  convertBytesToMB as convertBytes,
  computeThroughput as convertTimeToMBPerSecond,
  saveKeyToFile,
  readKeyFromFile,
};
