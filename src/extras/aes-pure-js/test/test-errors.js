  ...
  "test-errors-key-size": function (test) {
    for (var i = 0; i < keySizes.length; i++) {
      test.throws(
        ...
        function (error) {
          return (
            error.message === "invalid key size (must be 16, 24 or 32 bytes)"
          );
        },
    ...
    for (var i = 1; i < 17; i++) {
      for (var j = 1; j < 17; j++) {
        ...
        test.throws(
          ...
          "invalid plaintext (invalid segment size) failed to throw an error " +
            i +
            " " +
            j
        );
  ...
  "test-errors-counter": function (test) {
    ...
    for (var i = 0; i < textSizes.length; i++) {
      test.throws(
        ...
        function (error) {
          return (
            error.message === "invalid counter bytes size (must be 16 bytes)"
          );
        },
